...
void HcalHBHEMuonAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (_Muon.isValid() && barrelRecHitsHandle.isValid() && 
    ...
    for (reco::MuonCollection::const_iterator RecMuon = _Muon->begin(); RecMuon!= _Muon->end(); ++RecMuon)  {
      ...
      edm::LogInfo("HBHEMuon") << "after muon pT cut accept: " << accept<<std::endl;
      ...
      ismediummuon_.push_back(muon::isMediumMuon(*RecMuon)); //(*mu).isMediumMuon);
      //istightmuon_.push_back(muon::isTightMuon(*RecMuon));
	...
	eHcalDepth[i] = eHcalDepthHot[i] = activeL[i] = activeHotL[i] = eHcalDepth1[i] = eHcalDepth2[i] = eHcalDepth3[i] = eHcalDepth4[i] = eHcalDepth5[i] = eHcalDepth6[i] = eHcalDepth7[i] = eHcalDepth8[i] = -10000; 
      }
	...
	if (trackID.okHCAL) {
	  ...
	  for (unsigned int i=0; i<ehdepth.size(); ++i) {
	    ...
	    activeLengthTot += actL;
            ...
            //DetId id1(hcid1.rawId());

            //std::vector< typename T::const_iterator> hit1;
            //spr::findHit(hbhe, id1, hit1, false);
            //for (unsigned int ihit=0; ihit<hit1.size(); ihit++) energy1[ehdepth[i].second-1]= + spr::getRawEnergy(hit1,useRaw_);


            /*
            DetId det_rawID(hcid0.rawId());
            std::vector<DetId> ids_east = theHBHETopology->east(det_rawID);
            std::vector<DetId> ids_west = theHBHETopology->west(det_rawID);
            std::vector<DetId> ids_north = theHBHETopology->north(det_rawID);
            std::vector<DetId> ids_south = theHBHETopology->south(det_rawID);

            for(int hh = 0; hh< (int)ids_east.size(); hh++){
            std::vector< typename T::const_iterator> hit_east = spr::findHit(hbhe, ids_east.at(hh));
              for (unsigned int ihit=0; ihit<hit_east.size(); ihit++) energy1[ehdepth[i].second-1]= + spr::getRawEnergy(hit_east[ihit],useRaw_);
            }

            for(int hh = 0; hh< (int)ids_west.size(); hh++){
            std::vector< typename T::const_iterator> hit_west = spr::findHit(hbhe, ids_west[hh]);
              for (unsigned int ihit=0; ihit<hit_west.size(); ihit++) energyW[ehdepth[i].second-1]= + spr::getRawEnergy(hit_west[ihit],useRaw_);
            }
      
            for(int hh = 0; hh< (int)ids_north.size(); hh++){
            std::vector< typename T::const_iterator> hit_north = spr::findHit(hbhe, ids_north[hh]);
              for (unsigned int ihit=0; ihit<hit_north.size(); ihit++) energyN[ehdepth[i].second-1]= + spr::getRawEnergy(hit_north[ihit],useRaw_);
            } 

            for(int hh = 0; hh< (int)ids_south.size(); hh++){
            std::vector< typename T::const_iterator> hit_south = spr::findHit(hbhe, ids_south[hh]);
              for (unsigned int ihit=0; ihit<hit_south.size(); ihit++) energyS[ehdepth[i].second-1]= + spr::getRawEnergy(hit_south[ihit],useRaw_);
            }
            */
	  ...
	  if (hotCell != HcalDetId()) {
	    ...
	    ietahot   = HcalDetId(hotCell).ieta();
	    iphihot   = HcalDetId(hotCell).iphi();
            //if (ieta< 29 && ieta>15 && iphi>62 && iphi<67){
               //accept = true;
	    hborhe = (std::abs(ietahot) == 16);
	    std::vector<std::pair<double,int> > ehdepth;
	    ...
	    for (unsigned int i=0; i<ehdepth.size(); ++i) {
	      ...
	      HcalSubdetector subdet0 = (hborhe) ? ((ehdepth[i].second >= depthHE) ? HcalEndcap : HcalBarrel) : subdet;
	      HcalDetId hcid0(subdet0,ietahot,iphihot,ehdepth[i].second);
	      double actL = activeLength(DetId(hcid0));
